...
import AnniversaryMode from './AnniversaryMode';
...
const MODES_LIST = [
  ...
  { key: 'anniversary', title: 'Anniversary Mode', desc: 'Special calculator just for Nush', icon: '🩷' },
...
export default function Calculator() {
  ...
  const handleEquals = useCallback(() => {
    ...
    if (lowerExpr === 'daystogether()') {
      ...
      overrideResult = days.toString();
      overrideValue = days;
    } else if (lowerExpr === 'nushsmile()' || lowerExpr === 'nushsmileintensity()') {
      overrideResult = '∞';
      overrideValue = Infinity;
    } else if (lowerExpr === 'iloveyou()') {
      overrideResult = 'Infinity & Beyond';
      overrideValue = Infinity;
    } else if (lowerExpr === 'hugcount()') {
      overrideResult = 'Not enough. Come here.';
      overrideValue = 0;
    } else if (lowerExpr === 'daysuntilnextanniversary()') {
      const now = new Date();
      // Monthly anniversary is on the 22nd of each month (Day 0: June 22, 2026; 3-Month: Sept 22; 4-Month: Oct 22)
      let targetYear = now.getFullYear();
      let targetMonth = now.getMonth();
      let targetDate = new Date(targetYear, targetMonth, 22, 0, 0, 0);

      // If today is past the 22nd of the current month, roll over to the 22nd of next month
      if (now.getTime() > targetDate.getTime()) {
        ...
        targetDate = new Date(targetYear, targetMonth, 22, 0, 0, 0);
      }

      const diffMs = targetDate.getTime() - now.getTime();
      const days = Math.max(0, Math.ceil(diffMs / (1000 * 60 * 60 * 24)));
      if (days === 0) {
        overrideResult = 'TODAY IS OUR ANNIVERSARY! ❤️💍';
      } else {
        overrideResult = `${days} day${days === 1 ? '' : 's'} until the 22nd`;
      }
      overrideValue = days;
    } else if (lowerExpr === 'firstdate()') {
      overrideResult = '2026-06-22';
      overrideValue = 20260622;
    }
    ...
    if (overrideResult !== null) {
      ...
      setPrimaryDisplay(overrideResult);
      setCurrentInput('');
      setJustEvaluated(true);
      setLastResult(overrideValue);
      setAns(overrideValue!);
      return;
    }
              ...
              >
                <div className="flex flex-col h-full">
                  <AnniversaryMode
                    ...
                    onClear={() => handleClear('AC')}
                    onBackspace={() => handleClear('DEL')}
                  />
                  <div className="p-4 mt-auto">
                    <button
                      onClick={() => {
                        const text = `Hey Nush, I just calculated this for you:\n\n${expressionDisplay} ${primaryDisplay}\n\n🩷 Yajat`;
                        if (navigator.share) {
                          navigator.share({
                            title: 'Special Calculation',
                            text: text,
                      ...
                      className="w-full bg-[var(--pink-deep)] text-white font-nunito font-bold py-3 rounded-xl shadow-lg hover:bg-[var(--pink)] transition-colors active:scale-95 flex items-center justify-center gap-2"
                    >
                      💌 Send this result to Nush
                    </button>
                  </div>
                </div>
              </motion.div>
